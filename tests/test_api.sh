...
# 5. Verify Update
run_test "Verify Update (GET /paste/$PASTE_ID)" 200 "curl -H \"Accept: application/json\" $BASE_URL/paste/$PASTE_ID"

# 6. Delete Paste
run_test "Delete Paste (DELETE /paste/$PASTE_ID)" 204 "curl -X DELETE $BASE_URL/paste/$PASTE_ID"

# 7. Verify Deletion (Should be 404)
run_test "Verify Deletion (GET /paste/$PASTE_ID)" 404 "curl -H \"Accept: application/json\" $BASE_URL/paste/$PASTE_ID"
...
if [ -n "$QUERY_PASTE_ID" ]; then
    ...
    run_test "Filter by ID (GET /paste?paste_id=$QUERY_PASTE_ID)" 200 "curl -H \"Accept: application/json\" \"$BASE_URL/paste?paste_id=$QUERY_PASTE_ID\""

    # Test filter by tag
    run_test "Filter by Tag (GET /paste?tag=$QUERY_TAG)" 200 "curl -H \"Accept: application/json\" \"$BASE_URL/paste?tag=$QUERY_TAG\""

    # Test filter by ID and Tag
    run_test "Filter by ID and Tag (GET /paste?paste_id=$QUERY_PASTE_ID&tag=$QUERY_TAG)" 200 "curl -H \"Accept: application/json\" \"$BASE_URL/paste?paste_id=$QUERY_PASTE_ID&tag=$QUERY_TAG\""

    # Test filter by user (should be 404 since we don't have users assigned)
    run_test "Filter by User (GET /paste?user=nonexistent)" 404 "curl -H \"Accept: application/json\" \"$BASE_URL/paste?user=nonexistent\""

    # Test Wrong Accept header for query params
    run_test "Query Filter - Wrong Accept (400)" 400 "curl -H \"Accept: text/html\" \"$BASE_URL/paste?tag=$QUERY_TAG\""
    ...
    run_test "Query Filter - No Params" 404 "curl -H \"Accept: application/json\" $BASE_URL/paste"

    # Cleanup
    curl -s -X DELETE $BASE_URL/paste/$QUERY_PASTE_ID > /dev/null
else
...
EXPIRED_RESP=$(curl -s -H "Content-Type: application/json" -H "Accept: application/json" -d "$EXPIRED_BODY" "$BASE_URL/paste")
EXP_PASTE_ID=$(echo "$EXPIRED_RESP" | grep -oP '"(id|paste_id)":\s*[^,}]+' | head -1 | cut -d':' -f2 | tr -d ' "')

if [ -n "$EXP_PASTE_ID" ]; then
    run_test "Read Expired Paste (GET /paste/$EXP_PASTE_ID)" 410 "curl -H \"Accept: application/json\" $BASE_URL/paste/$EXP_PASTE_ID"

    UPDATE_EXPIRED_BODY='{"content": "Trying to update expired"}'
    run_test "Update Expired Paste (PUT /paste/$EXP_PASTE_ID)" 410 "curl -X PUT -H \"Content-Type: application/json\" -H \"Accept: application/json\" -d '$UPDATE_EXPIRED_BODY' $BASE_URL/paste/$EXP_PASTE_ID"

    # Cleanup expired paste
    curl -s -X DELETE $BASE_URL/paste/$EXP_PASTE_ID > /dev/null
else
...
run_test "GET - Invalid ID (Non-numeric 422)" 422 "curl -H \"Accept: application/json\" $BASE_URL/paste/abc"
run_test "GET - Not Found (404)" 404 "curl -H \"Accept: application/json\" $BASE_URL/paste/999999"
...
# DELETE /paste/{id}
run_test "DELETE - Invalid ID (Non-numeric 422)" 422 "curl -X DELETE $BASE_URL/paste/abc"
run_test "DELETE - Not Found (404)" 404 "curl -X DELETE $BASE_URL/paste/999999"
